class MbandMembershipObserver < ActiveRecord::Observer
  def after_create(membership)    
    ...
    body    = %|
    Click <a href="#{APPLICATION[:url]}/mbands/#{membership.mband.id}">here</a> to view the mband profile.<br/>
    Click <a href="#{APPLICATION[:url]}/mband_memberships/accept/#{membership.membership_token}">here</a> to accept the invitation.<br/>
    Click <a href="#{APPLICATION[:url]}/mband_memberships/decline/#{membership.id}">here</a> to decline the invitation.
    |
